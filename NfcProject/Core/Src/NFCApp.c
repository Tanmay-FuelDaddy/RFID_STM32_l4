...
#define NDEF_NFC_READ                0U   /*!< NDEF menu read               */
#define NDEF_NFC_WRITE_MSG1          1U   /*!< NDEF menu write 1 record     */
#define NDEF_NFC_WRITE_MSG2          2U   /*!< NDEF menu write 2 records    */
#define NDEF_NFC_FORMAT_TAG          3U   /*!< NDEF menu format tag         */
#if NDEF_FEATURE_FULL_API             
#define NDEF_NFC_MAX_FEATURES        4U   /*!< Number of menu items         */
#else                                 
#define NDEF_NFC_MAX_FEATURES        1U   /*!< Number of menu items         */
#endif /* NDEF_FEATURE_FULL_API */    
#define NDEF_WRITE_FORMAT_TIMEOUT     10000U /*!< When write or format mode is selected,  returns back to read mode after a timeout */
#define NDEF_LED_BLINK_DURATION         250U /*!< Led blink duration         */ 
                                      
#define NFC_RAW_MESSAGE_BUF_LEN        8192 /*!< Raw message buffer len     */
                                      
#define NFC_ST_MANUFACTURER_ID        0x02U /*!< ST Manufacturer ID         */
 ...
 * 4-byte UIDs with first byte 0x*F are Fixed number, not unique, use for this APP
...
static uint8_t ndefTEXT[]         = "Welcome to ST NDEF NFC";
...
static uint8_t              ndefNFCFeature     = NDEF_NFC_READ;
static bool                 verbose             = false;

static uint8_t              rawMessageBuf[NFC_RAW_MESSAGE_BUF_LEN];

static void NFCNdef(rfalNfcDevice *nfcDevice);
...
static void NFCP2P( void );
ReturnCode  NFCTransceiveBlocking( uint8_t *txBuf, uint16_t txBufSize, uint8_t **rxBuf, uint16_t **rcvLen, uint32_t fwt );
...
bool NFCAppInit( void )
{
    ...
    if( err == ERR_NONE )
    {
        ...
        discParam.techs2Find           = RFAL_NFC_TECH_NONE;          /* For the APP, enable the NFC Technologies based on RFAL Feature switches */
...
void NFCP2P( void )
{
    ...
    err = NFCTransceiveBlocking( ndefInit, sizeof(ndefInit), &rxData, &rxLen, RFAL_FWT_NONE);
    if( err != ERR_NONE )
    {
        platformLog("failed.\r\n");
        return;
    ...
    err = NFCTransceiveBlocking(bufPayload.buffer, bufPayload.length, &rxData, &rxLen, RFAL_FWT_NONE);
    if( err != ERR_NONE )
    {
        platformLog("failed.\r\n");
        return;
    ...
    while(err == ERR_NONE) 
    {
        err = NFCTransceiveBlocking( ndefLLCPSYMM, sizeof(ndefLLCPSYMM), &rxData, &rxLen, RFAL_FWT_NONE);
        platformLog(".");
        platformDelay(50);
    }
    platformLog("\r\n Device removed.\r\n");
...
ReturnCode NFCTransceiveBlocking( uint8_t *txBuf, uint16_t txBufSize, uint8_t **rxData, uint16_t **rcvLen, uint32_t fwt )
{
    ReturnCode err;
...
static void NFCNdef(rfalNfcDevice *pNfcDevice)
{
    ReturnCode       err;
    ndefMessage      message;
    ...
    if( err != ERR_NONE )
    {
        ...
        if( ndefNFCFeature != NDEF_NFC_FORMAT_TAG)
        {
            return;
        }
    }
    ...
    switch( ndefNFCFeature )
    {
         ...
         */
        case NDEF_NFC_WRITE_MSG1:
            ndefNFCFeature = NDEF_NFC_READ; /* returns to READ mode after write */
         ...
         */
        case NDEF_NFC_FORMAT_TAG:
            ndefNFCFeature = NDEF_NFC_READ;
